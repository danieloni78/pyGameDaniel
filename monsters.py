# This class stores and manages the monster information and the monster instances
...
# Most/Atk Types: 0 - Normal, 1 - Water, 2 - Fire, 3 - Grass, 4 - Electric ,5 - Psychic, 6 - Rock, 7 - Flying, 8 - Fighting,  9 - Dragon
effectiveList = [[-1], [2, 6], [3], [1, 6], [1, 7], [5, 8], [2, 4, 7], [3, 8], [0, 6], [9]]
...
class monster:
    ...
    # Print the monster to the screen
        ...
        # If monster is not dead
            ...
            if self.action == 1:
                ...
                if self.moveType == 2:
                    ...
                    if self.steps % 10 == 0:
                        printFunctions.print_transparent(screen, self.movement[self.steps // 10],
                                                         (self.posX, self.posY), 128)
                    if self.steps % 10 == 1 or self.steps % 4 == 2:
                        printFunctions.print_transparent(screen, self.movement[self.steps // 10],
                                                         (self.posX, self.posY), 180)
                    else:
                        printFunctions.print_transparent(screen, self.movement[self.steps // 10],
                                                         (self.posX, self.posY), 150)

                else:
                    # Print the monster
                    screen.blit(self.movement[self.steps // 10], (self.posX, self.posY))
            ...
            if self.action == 2:
                # Makes Assassin transparent
                if self.moveType == 2:
                    printFunctions.print_transparent(screen, self.movement[self.steps // 10], (self.posX, self.posY),
                                                     120)
                else:
    ...
    # For certain monsters (Quick attack)
    ...
    # Addition to the quick attack; Jumping
    def jumping(self):
        self.inJump = True
        self.jumpvar = 14
        self.action = 2
    ...
    def drawHealthBar(self):
        ...
        pygame.draw.rect(screen, (0, 255, 0),
                         (self.basePosX, self.basePosY + self.height + 150, round(150 * hpRatio), 30))
    ...
    def calcDamage(self):
        ...
        self.target.hpRendeer = font.render(f'HP: {int(self.target.hp * 10)} / {int(self.target.maxHp * 10)}', True,
                                            (255, 255, 255))
    ...
    # Turn monster to a special attacker
    ...
    # Function to Print the monsters for the starting screen
    def printMoving(self):
        # 16 // 4 = 4 <- Out of Range
        if self.steps >= 39:
            self.steps = 0
    ...
    # Function to restore the HP Value
    def restore(self):
        self.hp = self.maxHp
        self.hpRendeer = font.render(f'HP: {self.hp * 10} / {self.maxHp * 10}', True, (255, 255, 255))
